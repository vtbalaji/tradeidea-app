#!/usr/bin/env python3
"""
EOD Technical Analysis Batch Job (Python + DuckDB + NSE)

Fetches EOD data from NSE (via jugaad-data), stores in DuckDB,
calculates technical indicators, and updates Firebase Firestore.
"""
...
def fetch_eod_data(symbol):
    """
    Fetch EOD data from DuckDB (with automatic NSE updates)
    Returns DataFrame with 730 days of data (2 years for weekly calculation)
    """
    import signal
    ...
    try:
        print(f'  📥 Fetching/updating data for {symbol}...')

        # Set timeout for NSE fetch (60 seconds)
        signal.signal(signal.SIGALRM, timeout_handler)
        signal.alarm(60)

        try:
            # First, update data from NSE (incremental)
            nse_fetcher.fetch_and_store(symbol)
        finally:
            # Cancel the alarm
            signal.alarm(0)
        ...
        # Prepare DataFrame in format expected by indicators
        # Convert lowercase columns to uppercase
        ...
        print(f'  ⏱️  Timeout: NSE fetch took too long, skipping...')
        return None
...
def analyze_symbols():
    ...
    print('🚀 Starting Technical Analysis (Python + DuckDB + NSE)\n')
    print('=' * 60)
    ...
    try:
        ...
        for i, symbol in enumerate(symbols):
            ...
            try:
                # Fetch data from DuckDB (with automatic NSE updates)
                ...
                if len(df) < 200:
                    print(f'  ⏭️  Skipping - insufficient data (only {len(df)} rows)')
                    fail_count += 1
                    continue
        ...
        print('\n' + '=' * 60)
        print('📊 DuckDB Statistics:')
        stats = nse_fetcher.get_stats()
        ...
        print(f'⏭️  Skipped: {skipped_count} symbols (no data)')
